...
interface AntigravityCredentialStore {
  accounts: Record<string, AntigravityCredential>;
}
...
interface AntigravityQuotaWindow {
...
function decodeJwtPayload(token: string | undefined): { email?: string; sub?: string; name?: string } | undefined {
  if (!token) return undefined;
  const payload = token.split('.')[1];
  if (!payload) return undefined;
  try {
    return JSON.parse(Buffer.from(payload, 'base64url').toString('utf8')) as { email?: string; sub?: string; name?: string };
  } catch {
    return undefined;
  }
}

function buildAccountId(email: string, authId?: string): string {
...
function userAgentArch(): string {
  return os.arch() === 'arm64' ? 'arm64' : 'amd64';
}

function codeAssistUrl(path: string): string {
...
function buildCodeAssistHeaders(endpoint: string, accessToken: string): Record<string, string> {
  return {
...
function loadCodeAssistPayload(): unknown {
  return {
...
function parseTierPlanName(tierId: string | undefined): string | undefined {
...
function parseQuotaWindow(bucket: unknown): AntigravityQuotaWindow {
...
function quotaFromValue(raw: unknown): AntigravityQuotaSummary {
...
async function postCodeAssist(accessToken: string, endpoint: string, payload: unknown): Promise<unknown> {
...
async function exchangeOAuthCode(session: OAuthSession, code: string): Promise<TokenResponse> {
  ...
  return parseJsonResponse<TokenResponse>(response, 'Antigravity OAuth token exchange');
}
...
async function refreshToken(refreshToken: string): Promise<TokenResponse> {
  ...
  return parseJsonResponse<TokenResponse>(response, 'Antigravity token refresh');
}
...
function accountFromTokenResponse(response: TokenResponse, profile?: GoogleUserInfo, existing?: AntigravityAccount): { account: AntigravityAccount; credential: AntigravityCredential } {
  if (response.error) {
...
function trackFromAccount(account: AntigravityAccount, id: TrackId, label: string, window: AntigravityQuotaWindow): QuotaTrack {
  return {
    id,
...
export class AntigravityProvider {
  ...
  async connect(): Promise<AntigravityAccount> {
  ...
  async refreshAll(): Promise<AntigravityAccount[]> {
    const accounts = await this.getAccounts();
    const refreshed: AntigravityAccount[] = [];
    for (const account of accounts) refreshed.push(await this.refreshAccount(account.id));
    return refreshed;
  }

  async refreshAccount(accountId: string): Promise<AntigravityAccount> {
    const account = await this.getAccount(accountId);
    const credential = await this.getCredential(accountId);
    if (!account || !credential) throw new Error('Antigravity account is not connected.');
    ...
    try {
      ...
      const nextAccount: AntigravityAccount = {
        ...account,
  ...
  async disconnect(): Promise<void> {
    ...
    const confirmed = await vscode.window.showWarningMessage(
      `Disconnect Antigravity account ${picked.account.email}? Extension-stored tokens and cached quota data will be deleted.`,
      { modal: true },
      'Disconnect',
    );
  ...
  private async ensureAccessToken(account: AntigravityAccount, credential: AntigravityCredential): Promise<AntigravityCredential> {
    const shouldRefresh = credential.expiryDate != null && credential.expiryDate <= now() + 60_000;
    if (!shouldRefresh) return credential;
    if (!credential.refreshToken) throw new Error('Antigravity refresh token is missing.');
    ...
    if (!nextCredential) throw new Error('Antigravity token refresh did not persist credentials.');
    return nextCredential;
  }

  private async upsertTokenResponse(response: TokenResponse, profile?: GoogleUserInfo, existing?: AntigravityAccount): Promise<AntigravityAccount> {
  ...
  private async getAccounts(): Promise<AntigravityAccount[]> {
    return this.context.globalState.get<AntigravityAccount[]>(ANTIGRAVITY_ACCOUNTS_KEY, []);
  }

  private async getAccount(accountId: string): Promise<AntigravityAccount | undefined> {
    return (await this.getAccounts()).find((account) => account.id === accountId);
  }

  private async saveAccount(account: AntigravityAccount): Promise<AntigravityAccount> {
    const accounts = await this.getAccounts();
  ...
  private async getCredential(accountId: string): Promise<AntigravityCredential | undefined> {
    const store = await this.getCredentialStore();
    return store.accounts[accountId];
  }

  private async getCredentialStore(): Promise<AntigravityCredentialStore> {
    const raw = await this.context.secrets.get(ANTIGRAVITY_SECRET_KEY);
    if (!raw) return { accounts: {} };
    try {
      const parsed = JSON.parse(raw) as AntigravityCredentialStore;
  ...
  private async saveCredentialStore(store: AntigravityCredentialStore): Promise<void> {
